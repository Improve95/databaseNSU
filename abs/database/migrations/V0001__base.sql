...
create table acl_sids (
    ...
    principal char(1) not null default 1,
    constraint unique_user_principal unique (sid, principal)
);

create table acl_object_identities (
    id bigint primary key generated by default as identity,
    object_id_class int references acl_classes(id) not null,
    object_id_identity int,
    parent_object_id int,
    foreign key (parent_object_id) references acl_object_identities(id)
        deferrable initially immediate,
    owner_sid int references acl_sids(id) on delete cascade not null,
    entries_inheriting bool not null default true,
    constraint unique (object_id_class, object_id_identity)
);

create table acl_entries (
    id bigint primary key generated by default as identity,
    acl_object_identity_id bigint references acl_object_identities(id) on delete cascade not null,
    acl_order int,
    sid bigint references acl_sids(id) on delete cascade not null,
    mask int not null,
    granting char(1) not null default 0,
    is_audit bool not null default true
);*/
